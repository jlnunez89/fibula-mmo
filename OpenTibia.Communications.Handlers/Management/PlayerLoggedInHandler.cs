// -----------------------------------------------------------------
// <copyright file="PlayerLoggedInHandler.cs" company="2Dudes">
...
namespace OpenTibia.Communications.Handlers.Management
{
    ...
    public class PlayerLoggedInHandler : BaseHandler
    {
        /// <summary>
        /// Gets the type of packet that this handler is for.
        /// </summary>
        public override byte ForPacketType => (byte)IncomingManagementPacketType.PlayerLogIn;
        ...
        {
            var playerLoginInfo = message.ReadManagementPlayerLoginInfo();

            var failure = LoginFailureReason.None;

            using (var otContext = new OpenTibiaDbContext())
            {
                var userRecord = otContext.Users.Where(u => u.Login == playerLoginInfo.AccountNumber && u.Passwd.Equals(playerLoginInfo.Password)).FirstOrDefault();
                var playerRecord = otContext.Players.Where(p => p.Account_Nr == playerLoginInfo.AccountNumber && p.Charname.Equals(playerLoginInfo.CharacterName)).FirstOrDefault();
                ...
                else
                {
                    ...
                    var anotherCharacterIsLoggedIn = otContext.Players.Where(p => p.Account_Nr == playerLoginInfo.AccountNumber && p.Online > 0 && !p.Charname.Equals(playerLoginInfo.CharacterName)).Any();
                ...
                if (failure == LoginFailureReason.None)
                {
                    try
                    {
                        ...
                        this.ResponsePackets.Add(new PlayerLoginSucessPacket(
                            ...
                            playerRecord.Charname,
                            (byte)(playerRecord.Gender == 1 ? 0x01 : 0x02),
                            guildInfo?.GuildName,
                            rankString,
                            guildMembershipInfo?.PlayerTitle,
                            vipCollection,
                            userRecord.Premium_Days + userRecord.Trial_Premium_Days,
                            false, // TODO: load this.
                            privileges));
                ...
                this.ResponsePackets.Add(new PlayerLoginRejectionPacket((byte)failure));
            }
        }
    }
}