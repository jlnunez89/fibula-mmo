// -----------------------------------------------------------------
// <copyright file="MultiUseEventRule.cs" company="2Dudes">
...
namespace OpenTibia.Server.Events.MoveUseFile
{
    ...
    /// <summary>
    /// Class that represents a multi use event rule.
    /// </summary>
    internal class MultiUseEventRule : MoveUseEventRule
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="MultiUseEventRule"/> class.
        /// </summary>
        ...
        public MultiUseEventRule(ILogger logger, IScriptApi scriptFactory, IList<string> conditionSet, IList<string> actionSet)
            : base(logger, scriptFactory, conditionSet, actionSet)
        {
            // Look for a IsType condition.
            var isTypeConditions = this.Conditions.Where(func => MoveUseEventRule.IsTypeFunctionName.Equals(func.FunctionName));
            ...
            {
                throw new ArgumentNullException($"Unable to find first {MoveUseEventRule.IsTypeFunctionName} function.");
            }

            if (secondTypeCondition == null)
            {
                throw new ArgumentNullException($"Unable to find second {MoveUseEventRule.IsTypeFunctionName} function.");
            }
        ...
        /// <summary>
        /// Gets the id of the item to use on.
        /// </summary>
