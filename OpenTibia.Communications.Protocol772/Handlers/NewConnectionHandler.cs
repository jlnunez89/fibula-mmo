...
namespace OpenTibia.Communications.Protocol772.Handlers
{
    ...
    public sealed class NewConnectionHandler : BaseHandler
    {
        ...
        public NewConnectionHandler(
            ...
            IOptions<GameListenerOptions> gameListenerOptions,
            IOptions<GameConfigurationOptions> gameConfigurationOptions)
            : base(logger)
        {
            ...
            gameListenerOptions.ThrowIfNull(nameof(gameListenerOptions));
            gameConfigurationOptions.ThrowIfNull(nameof(gameConfigurationOptions));

            Validator.ValidateObject(protocolConfigurationOptions, new ValidationContext(protocolConfigurationOptions), validateAllProperties: true);
            Validator.ValidateObject(gameListenerOptions, new ValidationContext(gameListenerOptions), validateAllProperties: true);
            Validator.ValidateObject(gameConfigurationOptions, new ValidationContext(gameConfigurationOptions), validateAllProperties: true);

            this.ApplicationContext = applicationContext;

            this.ProtocolConfiguration = protocolConfigurationOptions.Value;
            this.GameListenerOptions = gameListenerOptions.Value;
            this.GameConfiguration = gameConfigurationOptions.Value;
        }
        ...
        public GameListenerOptions GameListenerOptions { get; }
        ...
        public override IEnumerable<IOutgoingPacket> HandleRequest(INetworkMessage message, IConnection connection)
        {
            ...
            foreach (var character in charactersFound)
            {
                charList.Add(new CharacterListItem(
                    character.Name,
                    IPAddress.Parse(this.GameListenerOptions.IpAddress),
                    this.GameListenerOptions.Port,
