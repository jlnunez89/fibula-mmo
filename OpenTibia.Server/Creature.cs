...
namespace OpenTibia.Server
{
    public abstract class Creature : Thing, ICreature, ICombatActor
    {
            ...
            ushort manapoints = 0)
        {
            ...
            OnLocationChanged += CheckPendingActions;                    // Are we in range with our pending action?
        ...
        public void SetAttackTarget(uint targetId)
        {
            ...
            if (targetId == 0)
            {
                ...
                if (AutoAttackTargetId != 0)
                {
                    ...
                    {
                        attackTarget.OnLocationChanged -= CheckAutoAttack;
                    }
        ...
        public void WalkTo(Direction direction)
        {
            lock (_enqueueWalkLock)
            {
                ...
                WalkingQueue.Enqueue(new Tuple<byte, Direction>(NextStepId, direction));
                Game.Instance.SignalWalkAvailable();
            }
        }
        ...
        {
            var timeDiff = Cooldowns[type].Item1 + Cooldowns[type].Item2 - currentTime;
        ...
        public void UpdateLastStepInfo(byte lastStepId, bool wasDiagonal = true)
        {
            ...
            Cooldowns[CooldownType.Move] = new Tuple<DateTime, TimeSpan>(Game.Instance.MovementSynchronizationTime, TimeSpan.FromMilliseconds(1000 * totalPenalty / (double)Math.Max(1, (int)Speed)));
