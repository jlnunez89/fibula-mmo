...
namespace OpenTibia.Security.Encryption
{
    ...
    public static class Rsa
    {
        ...
        public static bool Decrypt(ref byte[] buffer, int position, int length, bool useCipKeys)
        {
            if (useCipKeys)
            {
                return DecryptUsigCipSoftKeys(ref buffer, position, length);
            }

            return DecryptUsigOpenTibiaKeys(ref buffer, position, length);
        }
        ...
        /// <param name="length">The length of the message.</param>
        /// <returns>True if the decryption is deemed successful, false otherwise.</returns>
        public static bool DecryptUsigOpenTibiaKeys(ref byte[] buffer, int position, int length)
        {
            ...
            position = length - BufferLength;

            Array.Copy(buffer, position, temp, 0, BufferLength);
            ...
            Array.Copy(GetPaddedValue(output), 0, buffer, position, BufferLength);
        ...
        /// <param name="length">The length of the message.</param>
        /// <returns>True if the decryption is deemed successful, false otherwise.</returns>
        public static bool DecryptUsigCipSoftKeys(ref byte[] buffer, int position, int length)
        {
            if (length - position != BufferLength)
            {
                return false;
