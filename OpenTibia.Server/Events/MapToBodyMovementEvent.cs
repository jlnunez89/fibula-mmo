// -----------------------------------------------------------------
// <copyright file="MapToBodyMovementEvent.cs" company="2Dudes">
...
namespace OpenTibia.Server.Events
{
    ...
    /// <summary>
    /// Internal class that represents a movement event that happens from the map to the player's inventory.
    /// </summary>
    internal class MapToBodyMovementEvent : BaseMovementEvent
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="MapToBodyMovementEvent"/> class.
        /// </summary>
        ...
        /// <param name="toCreatureId">The id of the creature to which the movement is happening.</param>
            ...
            : base(logger, game, connectionManager, creatureFinder, creatureRequestingId, evaluationTime)
        {
            ...
            var onPassAction = new GenericEventAction(() =>
            {
                bool moveSuccessful = thingMoving is IItem item &&
                                      ...
                                      this.Game.PerformItemMovement(item, fromTile, targetPlayer.Inventory[(byte)toCreatureSlot] as IContainerItem, toIndex: 0, amountToMove: amount, requestorCreature: this.Requestor);
